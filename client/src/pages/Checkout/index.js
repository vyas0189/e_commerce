...
const Checkout = () => {
    ...
    const [formDetails, setFormDetails] = useState({
      ...
      const setValues = () => {
        return (
            <>
                <h1>{`${user.address} ${user.address2 ? user.address2 : ''} ${user.city} ${user.state} ${user.zip}`}</h1>
            </>
        )
      }
        ...
        <div>
            ...
            <Form>
                <Form.Check onClick={(e) => {
                    setShow(1);
                }} checked={show === 1} custom type="radio" id="custom-radio" label="Current Address"/>
                <Form.Check onClick={(e) => {
                    setShow(2)
                }} checked={show === 2} custom type="radio" id="custom-radio" label="Other Address"/>
                {show === 1 && setValues()}
                {show === 2 && secondForm()}
                    <Form.Group>
